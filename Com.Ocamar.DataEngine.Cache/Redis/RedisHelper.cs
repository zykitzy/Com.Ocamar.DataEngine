...
namespace Com.Ocamar.DataEngine.Cache.Redis
{
    ...
    public class RedisHelper
    {
        ...
        public IConnectionMultiplexer GetConnectionRedisMultiplexer()
        {
            if ((_connMultiplexer == null || !_connMultiplexer.IsConnected))
            {
                lock (Locker)
                {
                    if ((_connMultiplexer == null || !_connMultiplexer.IsConnected))
                    {
                        _connMultiplexer = ConnectionMultiplexer.Connect(_redisPath);
                    }
                }
            }
        ...
        {
            _connMultiplexer = ConnectionMultiplexer.Connect(_redisPath);
            DefaultKey = ConfigurationManager.AppSettings["RedisKey"];
            RegisterEvent();
        }
        /// <summary>
        /// 重载构造器
